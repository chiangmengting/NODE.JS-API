...
app.get('/chatList', function (req, res) {
    ...
    db.queryAsync(`SELECT mb_chat.*,MR_number,MR_name,MR_pic FROM mb_chat LEFT JOIN mr_information ON MR_number = myTo OR MR_number = myFrom WHERE myFrom = "${req.session.memberId}" OR myTo = "${req.session.memberId}" ORDER BY created_at ASC`)
        .then(results => {
            ...
            return db.queryAsync(`SELECT * FROM mb_chat WHERE myTo = "${req.session.memberId}" AND myRead = 0`);
        })
        .then(results => {
